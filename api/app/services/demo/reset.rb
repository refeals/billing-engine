module Demo
  # Wipes the billing data and seeds it again. This is the one path that deletes audit
  ...
  class Reset < ApplicationService
    ...
    # Not billing data: a reset that reseeds keeps the demo user and whoever is signed in.
    ACCOUNTS = %w[users sessions].freeze

    # `reseed: false` only wipes, accounts included (used by the seed specs to clean up
    # after themselves).
    ...
    def call
      ...
      ActiveRecord::Base.transaction do
        ...
        (connection.tables - kept_tables).each { |table| connection.execute("DELETE FROM #{connection.quote_table_name(table)}") }
