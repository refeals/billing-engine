...
2. **Two domains**, for example `billing.example.com` (web) and `api.billing.example.com`
...
| Type | Name | Value |
|---|---|---|
| A | `billing.example.com` | VPS IP |
| A | `api.billing.example.com` | VPS IP |

Wait until `dig +short api.billing.example.com` returns the IP; Let's Encrypt validation
fails until it does.
...
- **Environment**:
  ...
  WEB_ORIGIN=https://billing.example.com
...
- **Domains → Add Domain**: host `api.billing.example.com`, path `/`, **container port
  `3000`**, HTTPS on, certificate **Let's Encrypt**.
- **Deploy**. The first boot runs the seeds; the logs show `db:prepare`, then Puma
  listening on port 3000.

Check: `curl https://api.billing.example.com/up` answers 200 and
`curl https://api.billing.example.com/api/v1/dashboard/summary` returns the seeded figures
...
- **Environment → Build-time arguments** (not runtime variables: Vite reads it while
  ...
  ```
  VITE_API_URL=https://api.billing.example.com/api/v1
  ```

- **Domains → Add Domain**: host `billing.example.com`, path `/`, **container port `80`**,
  HTTPS on, Let's Encrypt.
- **Deploy**, then open `https://billing.example.com`: the dashboard shows
...
| `WEB_ORIGIN` | api | runtime | `https://billing.example.com` |
| `RAILS_LOG_LEVEL` | api | runtime | `info` |
| `VITE_API_URL` | web | **build** | `https://api.billing.example.com/api/v1` |
...
4. Every scenario passes against the live API:
   `curl -X POST https://api.billing.example.com/api/v1/simulator/scenarios/<key>/run`.
